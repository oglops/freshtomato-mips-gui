...
createFieldTable('', [
	...
	<ul>
		...
		</li><li><b>MySQL 程序路径</b> - 包含有 mysqld 等可执行文件的目录路径，请不要包括可执行文件名 "/mysqld"
		</li><li><b>保持连接</b> - 如果启用, mysqld 将会按照指定的频率被检查，并且崩溃后会被自动重启.
		</li><li><b>数据目录和临时目录.</b> - 警告! 请勿使用 NAND 闪存作为数据目录或临时目录.
	</li></ul>
</div>
</div>
