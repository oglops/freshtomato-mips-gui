...
/* BBT-END */
	...
	<ul>
		...
		</li><li><b>Transmission 程序路径</b> 到包含 transmission-daemon 等程序的路径.
		</li><li><b>保持活动</b> - 开启后,系统将在指定时段检查 transmission-daemon 运行状况并在其崩溃后将其重新启动.
		</li><li><b>监听端口</b> - torrent 客户端使用的端口.确保此端口不被占用.
		</li><li><b>自动添加 .torrents</b> - 在下载目录中搜索并自动添加.torrent 文件，可以在"自定义配置"中的'watch-dir'选项处添加额外的监控文件夹".
	</li></ul>
</div>
...
createFieldTable('', [
	...
	<ul>
		...
		</li><li><b>需要身份验证</b> - <b><i>高度推荐</i></b>. GUI将提示输入用户名/密码.
		</li><li><b>允许远程访问</b> - 此选项将开启 Transmission GUI 的 WAN 端访问端口并且允许外网访问GUI.
	</li></ul>
</div>
...
createFieldTable('', [
	...
	<ul>
		...
		</li><li><b>上传队列</b> - 开启后将限制同时上传/做种的最大数目.
	</li></ul>
</div>
