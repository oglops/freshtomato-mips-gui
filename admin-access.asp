...
<script type="text/javascript">
var m = [
	{ title: '访问设置', name: 'f_http_local', type: 'select', options: [[0,'Disabled'],[1,'HTTP']
/* HTTPS-BEGIN */
		,[2,'HTTPS']
		,[3,'HTTP &amp; HTTPS']
/* HTTPS-END */
...
/* HTTPS-END */
	...
	{ title: '界面文件目录', name: 'web_dir', type: 'select',
		options: [['default','默认: /www'], ['jffs', '自定义: /jffs/www (仅限有经验者使用!)'], ['opt', '自定义: /opt/www (仅限有经验者使用!)'], ['tmp', '自定义: /tmp/www (仅限有经验者使用!)']], value: nvram.web_dir, suffix: ' <small>更改前请考虑清楚再做决定!<\/small>' },
...
createFieldTable('', [
	{ title: '允许远程管理的<br />IP 地址', name: 'f_rmgt_sip', type: 'text', maxlen: 512, size: 64, value: nvram.rmgt_sip,
		suffix: '<br /><small>("空白" 不限制,可单一IP或范围;例: "1.1.1.1", "1.1.1.0/24", "1.1.1.1 - 2.2.2.2" 或 "me.example.com")<\/small>' },
...
<script type="text/javascript">
createFieldTable('', [
	{ title: '用户名', name: 'http_username', type: 'text', maxlen: 20, value: nvram.http_username, suffix: '&nbsp;<small>(空白则用户名为 "admin")<\/small>' },
	{ title: '允许网页登陆为"管理员"', name: 'f_http_root', type: 'checkbox', value: nvram.http_root == 1 },
	null,
	{ title: '密码', name: 'set_password_1', type: 'password', maxlen: 60, value: '**********' },
		{ title: '<i>(再次输入密码)<\/i>', indent: 2, name: 'set_password_2', type: 'password', maxlen: 60, value: '**********' }
]);
</script>
</div>
